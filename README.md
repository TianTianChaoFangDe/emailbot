...
| `今日` | 今天日程 |
| `日程` | 未来 7 天日程 |
| `待确认` | 所有待你安排/确认的事(列表, 互不阻塞) |
| `取消` | 跳过待确认问题(多条时会先问你是哪一个) |
| `9月20号有什么安排` / `下周有什么` / `9月15到20号有哪些事` | 按指定日期/区间查日程 |
| `明天下午3点字节跳动一面` | 自然语言加日程 |
| `周五 19:00-21:00 笔试` | 带时间段的日程 |
| `把字节的面试改到后天下午3点` | 修改日程(没说新时间时会反问你; 保留原时长平移) |
| `删除明天的笔试` | 删除日程(会先让你确认, 回复「确认」生效) |
| `字节那个安排在明天下午3点` / `取消第二个` | 回答/跳过某条待确认(自然语言点名) |

收到含时间窗口(如"链接48小时内有效")或时间不明的日程邮件时, bot 会把它们放进待确认列表
并逐条发消息问你。多条待确认**互不阻塞**: 随时可以发 `待确认` 查看全部, 用自然语言点名回答
任意一条(如 `腾讯那个安排在周五下午2点`), 或引用 bot 的某条问题消息直接回复时间。
